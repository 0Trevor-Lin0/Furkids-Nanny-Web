...
const peopleList = JSON.parse(localStorage.getItem("favoriteNanny"));
let filteredPeople = [];
let genderData = [];
...
// 初始畫面渲染，抓取localStorage資料放入CARD中
    ...
    dataPanel.innerHTML += `<div class="col-12 col-md-3 mb-3">
            <div class="card">
              ...
              <div class="card-body text-center">
                ...
                <a class="btn btn-info myFavorite" id = '${item.id}'>Remove</a>
              </div>
            </div>
          </div>`
  });
...
function removeMyFavorite(id) {
  ...
  const peopleIndex = peopleList.findIndex(item => {
    return item.id === id  //因為字串與數字型態不同所以一定要先把id數字化
  }) //找到id在陣列中的位置

  peopleList.splice(peopleIndex, 1)

  localStorage.setItem('favoriteNanny', JSON.stringify(peopleList))
  renderPaginator(peopleList.length)
  renderPeopleList(getPeopleByPage(peopleList))
}
...
dataPanel.addEventListener("click", function onClick(event) {
  if (event.target.matches(".person-pic")) {
    ...
    removeMyFavorite(Number(event.target.id))
  }
});
